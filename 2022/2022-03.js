...
let inp=`QLFdFCdlLcVqdvFLnFLSSShZwptfHHhfZZZpSwfmHp
...
for(let i=1;i<26;++i){
    pr[String.fromCharCode(65+i)]=27+i
    pr[String.fromCharCode(97+i)]=1+i
}
...
for(let i=0;i<inp.length;i+=3){
    for(let j of inp[i]){
        if(inp[i+1].includes(j) && inp[i+2].includes(j)){
            s+=pr[j];
            break;
        }
    }
