...
let inp2=
`Blueprin 1: Each ore robot costs 3 ore. Each clay robot costs 3 ore. Each obsidian robot costs 2 ore and 15 clay. Each geode robot costs 3 ore and 9 obsidian.
...
let total=0
for(let bli in bl){
    let opt=optimize(bli,24)
    l('bl',bli,opt)
    total+=opt*bl[bli].n
}
l('part 1 total',total)

total=1
for(let bli in [0,1,2]){
    let opt=optimize(bli,32)
    l('bl',bli,opt)
    total*=opt
}
...
function optimize(i,min=24,robots=[1,0],res=[0,0,0,0],planned=null){
    ...
    if(new_robot!==null){
        ...
        if(new_robot<3 && new_robot==robots.length-1) robots.push(0)
    }
    if(planned===null){
        //split recurse
        let max=0
