...
let inp=`/dev/grid/node-x0-y0     85T   72T    13T   84%
...
inp=inp.split`\n`
let a=[],gx,gy,ex,ey,pg,cnt=0
inp.forEach((r,i)=>{
    let m=r.match(/-x(\d+)-y(\d+)\s+(\d+)T\s+(\d+)T\s+(\d+)T/)
    a[+m[1]] || (a[+m[1]]=[])
    let ch='.'
    if(+m[4]==0){
        ex=+m[1];ey=+m[2]
...
let lx=a.length-1, ly=a[lx].length-1
a[lx][0]='G'
gx=lx;gy=0
vis()
...
function vis(){
    for(let i=0; i<a.length; ++i) l(a[i].join``)
}

function shp(x1,y1,x2,y2){
    let q=[[x1,y1,[]]],v=new Set(x1+','+y1)
    while(q.length){
        let [x,y,p]=q.shift()
        for(let [dx,dy] of [[1,0],[0,1],[-1,0],[0,-1]]){
            ...
            if(a[nx][ny]!='.') continue
            if(v.has(nx+','+ny)) continue
            if(nx==x2 && ny==y2) return [...p , [nx,ny]]
            v.add(nx+','+ny)
