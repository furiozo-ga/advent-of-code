...
let scr=[]
for(let i=0;i<6; ++i) scr[i]=Array(50).fill(' ')

inp=inp.split`\n`
inp.forEach((r,i)=>{
    r=r.split` `
    if(r[0]=='rect') [r[1],r[2]]=r[1].split`x`.map(Number)
    else{
        r[2]=+r[2].slice(2)
        r[3]=+r[4]
        r.pop()
...
l(scr.map(r=>r.join``).join``.match(/#/g).length)
scr.vis()
...
function exec(i){
    ...
    if(i[1]=='row'){
        scr[i[2]]=[ ...scr[i[2]].slice(-i[3]), ...scr[i[2]].slice(0,-i[3]) ]
        return
    }
    let tmp=[]
    for(let j=0; j<6; ++j) tmp[(j+i[3])%6]=scr[j][i[2]]
    for(let j=0; j<6; ++j) scr[j][i[2]]=tmp[j]
}