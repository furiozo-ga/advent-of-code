...
let inp1=`bot 147 gives low to bot 67 and high to bot 71
...
while(more){
    ...
    for(let i=0 ; i<b.length ;++i){
        if(b[i].v.length==2){
            if(Math.max(...b[i].v)==61 && Math.min(...b[i].v)==17){
                l('part 1',i)
            }
            pass(Math.max(...b[i].v) , b[i][1])
            pass(Math.min(...b[i].v) , b[i][0])
            b[i].v=[]
            more=true
        }
    }
...
// l(o)
l('part 2',o[0][0]*o[1][0]*o[2][0])
function pass(val,dst){
    if(1/dst<0){
        o[-dst] || (o[-dst]=[])
        o[-dst].push(val)
    }else{
