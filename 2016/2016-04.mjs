...
let inp1=`aaaaa-bbb-z-y-x-123[abxyz]
a-b-c-d-e-f-g-h-987[abcde]
not-a-real-room-404[oarel]
totally-real-room-200[decoy]`

let inp2=`fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
for(let i=0; i<inp.length; ++i){
    ...
    inp[i][1]=+inp[i][1]
    let ls={},lst=[]
    inp[i][0].replaceAll('-','').split``.forEach(e => {
        ls[e] || (ls[e]=0)
        ls[e]++
    });
    ...
    lst=lst.slice(0,5)
    if(m[3] == lst.map(e=>e[0]).join``) sum+=+m[2]
    // l(m[3],lst.map(e=>e[0]).join``)
    inp[i].push(lst)

    l(dec(m[1],+m[2]),m[2])
}
l(sum)
// l(dec('qzmt-zixmtkozy-ivhz',343))
function dec(s,n){
    let res=[]
    for(let i=0; i<s.length ;++i){
        ...
        res.push(String.fromCharCode('a'.charCodeAt()+((s[i].charCodeAt()-'a'.charCodeAt()+n)%26)) )
    }
    return res.join``
}