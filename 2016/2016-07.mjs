...
// l(tls('ioxxoj[asdfgh]zxcvbn'))

let inp1=`wysextplwqpvipxdv[srzvtwbfzqtspxnethm]syqbzgtboxxzpwr[kljvjjkjyojzrstfgrw]obdhcczonzvbfby[svotajtpttohxsh]cooktbyumlpxostt
...
function tls(s){
    if(/\[\w*(\w)(?!\1)(\w)\2\1/.test(s)) return false
    if(/(?:^|\])\w*(\w)(?!\1)(\w)\2\1/.test(s)) return true
    return false
}

function ssl(s){
    if(/(?:^|\])\w*(\w)(?!\1)(\w)\1.*\[\w*\2\1\2/.test(s)) return true
    if(/\[\w*(\w)(?!\1)(\w)\1\w*\].*\2\1\2\w*(?:\[|$)/.test(s)) return true
    return false
}