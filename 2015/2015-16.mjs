...
let inp=`Sue 1: children: 1, cars: 8, vizslas: 7
...
Sue 500: cars: 1, perfumes: 6, vizslas: 1`.split`\n`
...
NS: for(let i=1; i<s.length ;++i){
    for(let p of Object.keys(s[i]))
        if(s[i][p]!=an[p]) continue NS
    l(i)
    break
}
NP: for(let i=1; i<s.length ;++i){
    for(let p of Object.keys(s[i])){
        switch(p){
            case 'cats':
            case 'trees':       if(s[i][p]<=an[p]) continue NP ; break
            case 'pomeranians':
            case 'goldfish':    if(s[i][p]>=an[p]) continue NP ; break
            default:            if(s[i][p]!=an[p]) continue NP
        }
    }
    l(i)
    break
