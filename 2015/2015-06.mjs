...
let re=/(.+?) (\d+),(\d+) through (\d+),(\d+)/g
  , m , g=[] , cnt=0


for(let i=0;i<1000;++i) g[i]=Array(1000).fill(0)

while(m=re.exec(inp)){
    let f
    if(m[1]=='turn on')  f=()=>1
    if(m[1]=='turn off') f=()=>0
    if(m[1]=='toggle')   f= v=>1-v
    for(let y=+m[3]; y<=+m[5] ;++y)
        for(let x=+m[2]; x<=+m[4] ;++x)
            g[y][x]=f(g[y][x])
}
...
while(m=re.exec(inp)){
    let f
    if(m[1]=='turn on')  f=v=> v+1
    if(m[1]=='turn off') f=v=> v<1 ? 0 : v-1
    if(m[1]=='toggle')   f=v=> v+2
    for(let y=+m[3]; y<=+m[5] ;++y)
        for(let x=+m[2]; x<=+m[4] ;++x)
            g[y][x]=f(g[y][x])
}
